...
- `useChallenge` - parameter that defines whether the remote server requires a challenge request header:
  - use `true` in case you're connecting to [Mobile Utility Server](https://github.com/wultra/mobile-utility-server) or a similar service.
...
The `useChallenge` property has been removed from `CertStoreConfiguration`. The challenge-based request signing is now always required — the library always sends a random challenge in the `X-Cert-Pinning-Challenge` request header and validates the ECDSA signature in the `X-Cert-Pinning-Signature` response header.
